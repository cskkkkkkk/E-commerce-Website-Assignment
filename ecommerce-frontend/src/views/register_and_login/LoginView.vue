<template>
  <div class="page-wrapper">
  <div class="login-wrapper">
    <el-form class="login-container" :rules="rules" ref="sizeForm" :model="sizeForm" label-width="80px">
      <el-form-item class="login-item" label="用户名" prop="userName">
        <el-input placeholder="请设置用户名" v-model="sizeForm.userName"></el-input>
      </el-form-item>
      <el-form-item class="login-item" label="密码" prop="password">
        <el-input placeholder="请设置密码" v-model="sizeForm.password" show-password></el-input>
      </el-form-item>
      <el-form-item class="login-item" label="账号类型" size="small">
        <el-radio-group v-model="sizeForm.accountType">
          <el-radio-button label="消费者"></el-radio-button>
          <el-radio-button label="商家"></el-radio-button>
        </el-radio-group>
      </el-form-item>
      <el-form-item class="login-button">
        <el-button type="primary" @click="onSubmit()" class="full-width-button">登录</el-button>
      </el-form-item>
      <el-form-item class="other-options">
        <div class="options">
          <el-button class="find-pwd"><u>找回密码</u></el-button>
          <el-button class="regiter" @click="jumpToRegister()"><u>注册账号</u></el-button>
          <el-button class="return-nav" @click="jumpToNav()"><u>返回导航页</u></el-button>
        </div>
      </el-form-item>
    </el-form>
  </div>
  </div>
</template>

<script>
import { defineComponent } from "vue";
import {postLoginInfomation} from "@/service/login_service";
export default defineComponent({
  data() {
    return {
      sizeForm: {
        userName: '',
        password: '',
        accountType: "消费者"
      },
      rules: {
        userName: [
          { required: true, message: '用户名不能为空', trigger: 'blur' },
          { min: 3, max: 20, message: '长度在 3 到 20 个字符', trigger: 'blur' }
        ],
        password: [
          { required: true, message: '密码不能为空', trigger: 'blur' },
          { min: 8, max: 20, message: '长度在 8 到 20 个字符', trigger: 'blur' }
        ]
      }
    };
  },
  methods: {
    onSubmit() {

           postLoginInfomation(this.sizeForm)
              .then(result => {
                console.log(result);
                let jwt = result.jwt;
                let msg = result.userID;
                if (jwt != null) {
                  localStorage.setItem("token", jwt);
                  localStorage.setItem("userInfo", JSON.stringify({
                    'userName': this.sizeForm.userName,
                    'userID': msg,
                    "phone": result.phone,
                    "email": result.email,
                    "address": result.address
                  }));
                  this.$message.success("登录成功，已进入个人空间");
                  this.$router.push("/top/mall")
                } else {
                  this.$message.error("登录失败，请检查输入信息");
                }
              })
               .catch(error => {
                 console.log("1");
                 console.error('Error during login:', error);  // 捕获并打印错误信息
                 this.$message.error("登录请求失败，请稍后重试");
               });
      },


    jumpToRegister()
    {
      this.$router.push("/register");
    },
    jumpToNav()
    {
      this.$router.push("/");
    }

  }
});
</script>

<style scoped>
.page-wrapper {
  background-color:#f4ecf4; /* 设置页面背景色 */
  height: 100vh; /* 使容器占满整个视口高度 */
}
.login-wrapper {
  display: flex;
  justify-content: center;
  align-items: center;
  height: 100vh; /* 使容器占满整个视口高度 */
}

.login-container {
  width: 90%;
  max-width: 400px; /* 设置最大宽度 */
  border-radius: 20px;
  padding: 20px;
  text-align: left;
  box-shadow: 0 4px 20px rgba(0, 0, 0, 0.1); /* 添加阴影 */
  background-color: white; /* 设置背景色 */
}

.login-item {
  width: 100%;
  margin-bottom: 20px; /* 为表单项添加间距 */
}

.login-button {
  width: 100%; /* 使按钮容器占满整个宽度 */
}

.full-width-button {
  width: 100%; /* 使按钮占满整个宽度 */
  background-color: orangered; /* 设置按钮背景颜色为橙色 */
  border-color: orangered; /* 设置按钮边框颜色为橙色（如果需要） */
  border-radius: 40px;
}

.other-options {
  display: flex;
  justify-content: flex-end; /* 右对齐选项 */
  margin-top: 5px; /* 添加顶部间距 */
}

.options {
  display: flex; /* 使用 flex 布局 */
}

.regiter,
.find-pwd,
.return-nav{
  border: none; /* 去掉按钮边框 */
  color: gray; /* 设置字体颜色为灰色 */
  font-size: 12px; /* 设置字体大小 */
  margin-left: 10px; /* 为按钮添加左间距 */
}

</style>
